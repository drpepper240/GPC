...
-- How does it work? Firstly, there are threads. No, not exactly, but CC Parallel API works and is easy to use and hard to misuse, and true threads don't and aren't. 
-- Note: various sleep and timer functions either do not work or aren't robust. My advise is to use "network timers": simple non-parallel program on a separate computer which sends messages on a specific channel.

-- Threads. There are at least three of them: first one receives network messages, second one is draws or redraws all GUI or single widgets, and the last processes user input (and, therefore, is sending messages). Just like in the IPhone, but better and free of charge.
...
--P_LEM					--laser emitter
--P_LEC					--laser emitter with camera (can send special kind of packet if getFirstHit() result is changed)
...
function GetControllerStateColor(controllerId)
	...
	if missing then 
		...
		return T_ctrlTempData[controllerId].state
	end
end
...
function DrawWidget(guiDataId)
	...
	elseif controllerType == "LASER_CAM" then
		DrawWidgetLaserCam(guiDataId)
	...
	if T_guiXy[cx] == nil then
		...
		if T_guiXy[cx][cy] == nil then
			...
			if sdata.guiData[guiDataId] == nil then
				...
				elseif guiType == "LASER_CAM" then
					ClickWidgetLaserCam(guiDataId)
...
function ClickWidgetLaserCam(guiDataId)
	local widget = sdata.guiData[guiDataId]
	local controllerId = sdata.guiData[guiDataId].controllerIds[1]
	if T_ctrlTempData[controllerId] == nil then
		PrintDbg("No temp data for controller_id "..controllerId, 2)
		return
	end
	
	--adding coordinates to target table
	if T_ctrlTempData[controllerId].hitX == nil or T_ctrlTempData[controllerId].hitY == nil or T_ctrlTempData[controllerId].hitZ == nil then
		return
	end
	table.insert(T_guiTempData[widget.tTGuiId].targetList, {x = T_ctrlTempData[controllerId].hitX, y = T_ctrlTempData[controllerId].hitY, z = T_ctrlTempData[controllerId].hitZ, comment = "SCAN"})	
	os.queueEvent("redraw")
end
